#!/usr/bin/env sh
TEMPLATE_CONFIG_FILE="/usr/local/share/nut_webgui/config.toml"
TEMPLATE_USERS_FILE="/usr/local/share/nut_webgui/users.toml"
...
DEFAULT_USERS_FILE="$DEFAULT_CONFIG_DIR/users.toml"
...
if [ "$CURRENT_UID" -eq "0" ]; then
    ...
    if [ -z "$NUTWG__CONFIG_FILE" -o "$NUTWG__CONFIG_FILE" = "$DEFAULT_CONFIG_FILE" ]; then
        if [ ! -e "$DEFAULT_CONFIG_FILE" ]; then
            install -m 664 -o "$APP_USER" -g "$APP_GROUP" "$TEMPLATE_CONFIG_FILE" "$DEFAULT_CONFIG_FILE"
        fi

        NUTWG__CONFIG_FILE="$DEFAULT_CONFIG_FILE"
    fi

    if [ -z "$NUTWG__AUTH__USERS_FILE" -o "$NUTWG__AUTH__USERS_FILE" = "$DEFAULT_USERS_FILE" ]; then
        if [ ! -e "$DEFAULT_USERS_FILE" ]; then
            install -m 664 -o "$APP_USER" -g "$APP_GROUP" "$TEMPLATE_USERS_FILE" "$DEFAULT_USERS_FILE"
        fi
    fi

    if [ -z "$NUTWG__SERVER_KEY" -o "$NUTWG__SERVER_KEY" = "$DEFAULT_SERVER_KEY" ]; then
...
export NUTWG__CONFIG_FILE
export NUTWG__DEFAULT_THEME
export NUTWG__LOG_LEVEL
export NUTWG__SERVER_KEY
export NUTWG__AUTH__ALLOW_ANONYMOUS_METRICS
export NUTWG__AUTH__USERS_FILE
export NUTWG__HTTP_SERVER__BASE_PATH
export NUTWG__HTTP_SERVER__LISTEN
export NUTWG__HTTP_SERVER__PORT
export NUTWG__HTTP_SERVER__WORKER_COUNT
export NUTWG__UPSD__ADDRESS
export NUTWG__UPSD__MAX_CONNECTION
export NUTWG__UPSD__NAME
export NUTWG__UPSD__PASSWORD
export NUTWG__UPSD__POLL_FREQ
export NUTWG__UPSD__POLL_INTERVAL
export NUTWG__UPSD__PORT
export NUTWG__UPSD__TLS_MODE
export NUTWG__UPSD__USERNAME

if [ "$CURRENT_UID" -eq "0" ]; then
    exec su -p -s "/bin/sh" -c 'exec "$@"' -- "$APP_USER" sh "$@"
else
    exec "$@"
fi
