...
pub trait Client {
  fn close(&mut self) -> impl std::future::Future<Output = Result<(), NutClientErrors>>;

  fn reconnect(&mut self) -> impl std::future::Future<Output = Result<(), NutClientErrors>>;

  fn get_ups_list(
    &mut self,
  ) -> impl std::future::Future<Output = Result<Vec<Ups>, NutClientErrors>>;

  fn get_cmd_list(
    &mut self,
    ups_name: &str,
  ) -> impl std::future::Future<Output = Result<Vec<Box<str>>, NutClientErrors>>;

  fn get_var_list(
    &mut self,
    ups_name: &str,
  ) -> impl std::future::Future<Output = Result<Vec<UpsVariable>, NutClientErrors>>;

  fn get_var(
    &mut self,
    ups_name: &str,
    var_name: &str,
  ) -> impl std::future::Future<Output = Result<UpsVariable, NutClientErrors>>;
}
