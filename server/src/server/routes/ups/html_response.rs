...
pub(super) async fn partial_page_variables(
  Path(ups_name): Path<String>,
  State(state): State<Arc<ServerState>>,
) -> impl IntoResponse {
  let ups_name = ups_name.as_str();
...
pub(super) async fn partial_page_info(
  Path(ups_name): Path<String>,
  State(state): State<Arc<ServerState>>,
) -> impl IntoResponse {
  let ups_name = ups_name.as_str();
