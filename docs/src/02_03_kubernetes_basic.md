...
> [!NOTE]
> This example covers the application deployment. Exposing the service outside
> the cluster requires a separate ingress configuration[^k8s_ingress].
...
Finally, create a Deployment to define the application state, including the
...
```
[^k8s_ingress]: [kubernetes.io/docs](https://kubernetes.io/docs/concepts/services-networking/ingress/)
