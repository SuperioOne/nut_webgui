...
`nut_webgui` can export UPS metrics in `OpenMetrics1.0`[^openmetrics] format.
The device statistics are available on the `/metrics` endpoint.
...
`nut_webgui` uses status names defined in RFC9271[^rfc9271]. Some UPS drivers
may have additional status names, but these are not currently encoded in the UPS
status metric.
...
set it to match your `poll_freq` value.
...
> [!NOTE]
> For other configuration options, see Prometheus's own documentation[^prometheus].

[^openmetrics]: [prometheus.io - OpenMetrics](https://prometheus.io/docs/specs/om/open_metrics_spec/#overview)
[^prometheus]: [prometheus.io - Configuration](https://prometheus.io/docs/prometheus/latest/configuration/configuration/)
[^rfc9271]: [RFC9271 - Status Symbols](https://www.rfc-editor.org/rfc/rfc9271.html#symbols)
