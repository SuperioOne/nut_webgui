...
When your NUT service runs outside the cluster, you can use a `Service` and an
`EndpointSlice` to create a cluster resource from the external NUT server.
This enables DNS-based discovery within the cluster (e.g., `nut-service`).
...
## Exposing the external NUT service
...
## Deploying nut_webgui

Configure `nut_webgui` to connect NUT server on `nut-service`.
    ...
    spec:
      containers:
        - name: nutweb
          ...
          env:
            - name: UPSD_ADDR
              value: nut-service # Kubernetes service name to connect to the NUT service.
            - name: UPSD_PORT
              value: "3493"
            - name: POLL_FREQ
              value: "20"
