...
When connecting to a NUT server that uses a certificate, the `nut_webgui`
container needs to trust the Root Certificate Authority (CA) that signed the
...
work if your `nut` package is built with `--with-openssl`.
...
2.  **Create a Self-Signed Root CA**
    ...
    certutil -S -d cert_db -n "My Root CA" \
             ...
             -t "CT,," -x -2 -v 24
    ```
    *   `-t "CT,,"`: Specifies this certificate trust attributes (CT -> Trusted
    ...
    *   `-v`: Number of months root certificate is valid for.
    ...
    > **Note:** `certutil` may ask, "Is this a CA certificate? [y/N]". You
    > **must** answer `y` to create a valid Root CA.
...
4.  **Create a Server Certificate Signing Request (CSR)**
    ...
    certutil -R \
      ...
      -o server.req
    ```
    > **Critical:** The Common Name (`CN`) and Subject Alternative Names
    > (`--extSAN`) must include the hostname(s) and IP address(es) that clients
    > will use to connect to the NUT server. This value must match what is
    > configured in `nut_webgui`'s `UPSD_ADDR` environment variable or the
    > `[upsd].address` field in `config.toml`.
